...
if(!$dataid) {
	...
	$q_fl = sql("SELECT f.id,f.name,f.status,fex.date_created,fex.fname, fex.lname, tl.name AS type_name, tu.name AS unit
FROM qi f
LEFT JOIN (
	SELECT fe.fid, MAX(fe.date_created) as date_created, MAX(fe.version) as version, ue.fname, ue.lname
	FROM qi_ext fe
	INNER JOIN user_ext ue ON fe.uid=ue.uid
	GROUP BY fe.fid, ue.fname, ue.lname
) fex ON f.id=fex.fid
...
ORDER BY f.name ASC");
	...
	if(!$c_fl) { $data = sysMsg(19); } else {
		...
		$data = '<table class="tablesorter" id="data-list"><thead><tr><th>Check List Name</th><th>Type</th><th>Unit</th><th>Authored by</th><th>Date Created</th><th>Status</th><th>Select</th></tr></thead><tbody>'.$data.'</tbody></table>';
	}	
} //get list
...
if($id && $dataid) {
	...
	$q_data = sql("SELECT `name` FROM `qi` WHERE id = '".$dataid."'");
	$r_data = mysqli_fetch_assoc($q_data); 
	$data = '<p><label>Checklist name:</label>'.$r_data['name'].'</p>';
	
	//get version information
	$q_version = sql("SELECT * FROM `qi_ext` WHERE `fid` = '".$dataid."' ORDER BY `version` DESC");
	...
	if(!$c_version) { $data = sysMsg(19); } else {
		while($r_version = mysqli_fetch_assoc($q_version)) {
			...
			//get number of responses per version
				$q_num = sql("SELECT COUNT(id) as count FROM `qi_responses` WHERE `feid` = '".$r_version['id']."'"); $r_num = mysqli_fetch_assoc($q_num);
				if($r_num['count']) {
					$res = ', '.$r_num['count'].' records';
					$q_date= sql("SELECT `date` FROM `qi_responses` WHERE `feid` = '".$r_version['id']."' ORDER BY `date` ASC LIMIT 0,1"); $r_date = mysqli_fetch_assoc($q_date);
...
if($id && $dataid && $datavid && ($norecord == '0')) {
	...
	$data .= '
		...
		';
	$form_exec = WEBSITE_LOC.'api/checklist-report-csv.php';

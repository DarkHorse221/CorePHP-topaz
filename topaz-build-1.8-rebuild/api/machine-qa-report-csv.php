...
include('../_constants/constants.php'); include('../_constants/db.php'); include('../_functions/strings.php'); include('../_functions/requests.php');
	...
	$q_ques = sql("SELECT `id`, `question`, `type` FROM `machine_qa_checklist_q` WHERE `ceid` = '".$vid."' ORDER BY `order` ASC");
	...
	//now get the machine for the checklist
	$q_mach = sql("SELECT m.name FROM machine_qa_checklist c LEFT JOIN machines m ON m.id=c.mid WHERE c.id = '".$vid."'");
	...
	$q_rec = sql("SELECT f.id, f.date, f.text, f.pass, ue.fname, ue.lname, f.ceid FROM machine_qa_responses f
	...
	while($r_rec = mysqli_fetch_assoc($q_rec)) {
			//for each record row display fixed response data
				...
				$data .= $csv_enclosed.str_replace('&#39;','\'',$mach_name).$csv_enclosed.$csv_separator;
				$data .= $csv_enclosed.str_replace('&#39;','\'',$r_rec['fname'].' '.$r_rec['lname']).$csv_enclosed.$csv_separator;
				$data .= $csv_enclosed.str_replace('&#39;','\'',$r_rec['pass']).$csv_enclosed.$csv_separator;
			...
			foreach($ques_arr as $k => $v) {
				$q_multi = sql("SELECT response FROM machine_qa_responses_ext WHERE gid = '".$r_rec['id']."' AND cqid = '".$k."'");
	...
	$out .= "Identifier,Date,Machine Name,Reported By,Pass,Comments,".$tbl_header.$csv_terminated;
	$out .= substr($data, 0, -1);
	
	$filename = 'Machine_QA'."_".date("d-m-Y_H-i-s",time()).".csv";

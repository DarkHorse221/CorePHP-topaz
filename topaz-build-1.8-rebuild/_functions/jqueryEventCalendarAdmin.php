<?php define('TOPAZ', true);
	...
	$q_ins = sql("SELECT e.id, e.name, e.groups, e.credits, e.mandatory, ei.id AS date_id, ei.start, ei.end FROM education e LEFT JOIN education_ins ei ON e.id=ei.eid WHERE e.tid='32' AND ei.start >= '".$start."' AND ei.end <= '".$end."'  ORDER BY ei.start DESC"); $c_ins = mysqli_num_rows($q_ins);
	...
	if($c_ins) {
		...
		while($r_ins = mysqli_fetch_assoc($q_ins)) {
			...
			$dates['allDay'] = false;
			if($r_ins['mandatory']) { $color = '#FF6666'; } else { $color = '#66CCFF'; }
			$dates['color'] = $color;
			$i++; $events[] = $dates;
		}
	} //$c_ins
